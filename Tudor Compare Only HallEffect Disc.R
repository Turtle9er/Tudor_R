...
while (i <= original_rows) {
  ...
  if (current_row$HallSamples < SAMPLES_PER_REV && i < original_rows) {
    next_row <- log_data[i + 1, ]
    
    # Check if this row and the next one form a complete 18-sample revolution
    is_valid_split <-                       (current_row$HallSamples + next_row$HallSamples == SAMPLES_PER_REV) &&
  ...
  if (current_row$HallSamples == SAMPLES_PER_REV) {
...
print(table(log_data$HallSamples))

cat("\n--- Sample Counts After Correction ---\n")
# This table should now ideally only show '18'
print(table(corrected_log_data$HallSamples))
}
...
rev_speed_data <- corrected_log_data %>%
  filter(HallSumTime > 0, HallSumTime2 > 0) %>%
  mutate(
    ...
    RevSpeedRange_kmh = pmax(HallSpeed_kmh, HallSpeed2_kmh) - 
                        pmin(HallSpeed_kmh, HallSpeed2_kmh),
...
rev_speed_data %>%
  select(revolution_id,  HallSpeed_kmh, HallSpeed2_kmh) %>%
  ...
  ggplot(aes(x = revolution_id, y = speed_kmh, color = sensor)) +
    ...
    labs(
      title = "2-Sensor Speeds Per Revolution Over Time",
...
per_sample_data <- corrected_log_data %>%
  ...
  mutate(
    ...
    speed_range_array = pmap(list(HallSpeed_kmh, HallSpeed2_kmh), ~ {
      temp_df <- tibble(s1=..1, s2=..2)
      pmax(temp_df$s1, temp_df$s2) - 
      pmin(temp_df$s1, temp_df$s2)
    }),
...
if (nrow(problem_revolutions) > 0) {
  ...
  per_sample_data %>%
    ...
    select(record_id, HallSpeed_kmh, HallSpeed2_kmh) %>%
    # Unnest the four list columns into a long format
    unnest(cols = c(HallSpeed_kmh, HallSpeed2_kmh)) %>%
    ...
    ggplot(aes(x = sample_number, y = speed_kmh, color = sensor)) +
      ...
      geom_point(size = 2.5) +
      labs(
        title = paste("Detailed 2-Sensor Speed Profile for Revolution #", record_to_plot),
...
sensor_speeds <- wide_speed_data %>%
  select(HallSpeed_kmh, HallSpeed2_kmh)
...
write.table((continuous_plot_data),  file="clipboard-16384", sep="\t", row.names=FALSE)

# --- 2. Generate the Continuous Line Graph ---
ggplot(continuous_plot_data, aes(x = global_sample_id, y = speed_kmh, color = sensor)) +
...
library(plotly)
library(ggplot2)
# 1. Store your ggplot object in a variable
p <- ggplot(continuous_plot_data, aes(x = global_sample_id, y = speed_kmh, color = sensor)) +
  geom_line(linewidth = 0.6, alpha = 0.8) +
  labs(
    title = "Continuous Speed Timeline for All Sensors",
    subtitle = "Plotting every sample from the entire data log",
...
# 2. Convert it to an interactive plot
ggplotly(p)
...
paired_plot_data <- paired_diff_data %>%
  ...
  select(record_id, Hall_Difference) %>%
  
  # Unnest both difference arrays
  unnest(cols = c(Hall_Difference)) %>%
